...
# # def test_create_checkout_success(client, session: Session):
# #     new_checkout = CheckoutCreate(
# #         checkout_date=date.today(), expected_return_date=date.today() + timedelta(days=7), member_id=1, copy_id=2
# #     )
# #     response = client.post("/checkout/", json=json.loads(new_checkout.model_dump_json()))
# #     print(response.text)
# #     assert response.status_code == 200
# #     assert response.json() == {}

# #     checkout: Checkout = session.exec(
# #         select(Checkout).filter(Checkout.member_id == 1).filter(Checkout.copy_id == 2)
# #     ).one()
# #     assert checkout.current_owner.id == new_checkout.member_id
# #     assert checkout.checkout_date == new_checkout.checkout_date
# #     assert checkout.expected_return_date == new_checkout.expected_return_date
...
#     assert response.status_code == 200
#     assert response.json() == {"message": f"Checkout id {checkout_id} deleted successfully"}
...
#     assert response.json() == {"detail": f"Checkout id {checkout_id} is not returned. Please make sure the book was returned before deleting the checkout"}
...
#     assert response.json() == {'detail': [{'type': 'missing', 'loc': ['body', 'checkout_date'], 'msg': 'Field required', 'input': {'expected_return_date': '2021-01-15', 'member_id': 1, 'copy_id': 1}, 'url': 'https://errors.pydantic.dev/2.6/v/missing'}]}
